...
class PluginShellcommandsShellcommand_Item extends CommonDBTM {
  ...
  'Phone', 'Printer'
      ...
      if (!$withtemplate) {
         if ($item->getType() == 'PluginShellcommandsShellcommand'
            ...
            return __('Associated item');
                 ...
                 && plugin_shellcommands_haveRight('shellcommands', 'r')) {
            if ($_SESSION['glpishow_count_on_tabs']) {
   ...
   public static function showForShellcommands(PluginShellcommandsShellcommand $shellcommand) {
      ...
      $canedit = $shellcommand->can($shell_id, 'w');
      $rand = mt_rand();

      if (!Session::haveRight("link", "r")
              || !$shellcommand->can($shell_id, 'r')) {
         return false;
      }
   ...
   static function showForItem(CommonDBTM $item, $withtemplate = '') {
      ...
      if ($number) {
         ...
         Ajax::updateItemOnSelectEvent("dropdown_name$randSelect", "command_name$randSelect", $CFG_GLPI["root_doc"]."/plugins/shellcommands/ajax/dropdownCommand.php", array('idtable' => $item->getType(),
             'value' => '__VALUE__',
             'itemID' => $ID,
             'countItem' => 1,
