...
class PluginManufacturersimportsPreImport extends CommonDBTM {
   ...
   */
   ...
   static function showImport($row_num,$item_num,$line,$output_type,$configID,$status,$imported) {
      ...
      if ($suppliername) {
         ...
         $ID = "";
         if ($_SESSION["glpiis_ids_visible"]||empty($line["name"])) $ID.= " (".$line["id"].")";
   ...
   */
   static function searchForm ($params) {
      ...
      $query.= " ORDER BY `glpi_plugin_manufacturersimports_configs`.`entities_id`, `glpi_plugin_manufacturersimports_configs`.`name`";
      ...
      echo "</td><td>";
      echo "<input type=\"submit\" name=\"typechoice\" class=\"submit\" value='".__s('Post')."' >";
         ...
         if ($p['start']<$numrows) {
            ...
            foreach ($toview as $key => $val) {
               ...
               if (!isset($searchopt["PluginManufacturersimportsPreImport"][$val]['nosort'])
                  ...
                  $linkto = "$target?itemtype=".$p['itemtype']."&amp;manufacturers_id=".$p['manufacturers_id']."&amp;imported=".$p['imported']."&amp;sort=".$val."&amp;order=".($p['order']=="ASC"?"DESC":"ASC").
