...
if (strpos($_SERVER['PHP_SELF'],"dropdownCertificates.php")) {
	$AJAX_INCLUDE=1;
...
$where=" WHERE  (`glpi_plugin_certificates_certificates`.`plugin_certificates_certificatetypes_id` = '".$_POST['plugin_certificates_certificatetypes_id']."')  
					AND `glpi_plugin_certificates_certificates`.`is_deleted` = '0' ";

if (isset($_POST["entity_restrict"])&&$_POST["entity_restrict"]>=0) {
	$where.=getEntitiesRestrictRequest("AND","glpi_plugin_certificates_certificates",'',$_POST["entity_restrict"],true);
} else {
	$where.=getEntitiesRestrictRequest("AND","glpi_plugin_certificates_certificates",'','',true);
}
...
if ($_POST['searchText']!=$CFG_GLPI["ajax_wildcard"])
	$where.=" AND `glpi_plugin_certificates_certificates`.`name` ".Search::makeTextSearch($_POST['searchText']);
