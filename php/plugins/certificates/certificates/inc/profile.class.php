...
class PluginCertificatesProfile extends CommonDBTM {
   ...
   static function getTypeName($nb=0) {
      return __('Rights management', 'certificates');
   }
   ...
   static function displayTabContentForItem(CommonGLPI $item, $tabnum=1, $withtemplate=0) {
      ...
      if ($item->getType()=='Profile') {
         ...
         $prof->showForm($item->getField('id'), array('target' => 
                           $CFG_GLPI["root_doc"]."/plugins/certificates/front/profile.form.php"));
      }
      return true;
   }
   ...
   static function changeProfile() {
      ...
      if ($prof->getFromDBByProfile($_SESSION['glpiactiveprofile']['id'])) {
         $_SESSION["glpi_plugin_certificates_profile"]=$prof->fields;
      } else {
         unset($_SESSION["glpi_plugin_certificates_profile"]);
      }
   }
	...
	function showForm ($ID, $options=array()) {
      ...
      $this->showFormHeader($options);
		...
		echo "<th colspan='4' class='center b'>".sprintf(__('%1$s - %2$s'), __('Rights management', 'certificates'),
         $prof->fields["name"])."</th>";
      ...
      echo "</tr>";
		...
		echo "<td>" . __('Associable items to a ticket') . " - " .
         PluginCertificatesCertificate::getTypeName(2) . "</td><td>";
