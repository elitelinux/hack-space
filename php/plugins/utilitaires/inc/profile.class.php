...
class PluginUtilitairesProfile extends CommonDBTM {

	static function getTypeName($nb=0) {

      return __('Rights management', 'utilitaires');
   }
   ...
   function getTabNameForItem(CommonGLPI $item, $withtemplate=0) {
      ...
      if ($item->getType()=='Profile' 
         ...
         return PluginUtilitairesUtilitaire::getTypeName(2);
      }
      return '';
   }
   ...
   static function displayTabContentForItem(CommonGLPI $item, $tabnum=1, $withtemplate=0) {
      ...
      if ($item->getType()=='Profile') {
         ...
         $prof->showForm($item->getField('id'), array('target' => 
                           $CFG_GLPI["root_doc"]."/plugins/utilitaires/front/profile.form.php"));
      }
      return true;
   }
   ...
   static function changeProfile() {
      ...
      if ($prof->getFromDBByProfile($_SESSION['glpiactiveprofile']['id']))
         $_SESSION["glpi_plugin_utilitaires_profile"]=$prof->fields;
      else
         unset($_SESSION["glpi_plugin_utilitaires_profile"]);
   }
	...
	function showForm ($ID, $options=array()) {
      ...
      $this->showFormHeader($options);
		...
		echo "<th colspan='2' class='center b'>".sprintf(__('%1$s - %2$s'), __('Rights management', 'utilitaires'),
         $prof->fields["name"])."</th>";
