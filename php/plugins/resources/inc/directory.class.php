         ...
         foreach ($searchopt[$itemtype] as $key => $val) {
            ...
            if (is_array($val)) {
               ...
               $query .= Search::addLeftJoin($itemtype, $itemtable, $already_link_tables,
                                          ...
                                          $searchopt["PluginResourcesDirectory"][$key]["joinparams"]);
      ...
      if ($result = $DB->query($query)) {
         ...
         if ($p['start']<$numrows) {
            ...
            foreach ($toview as $key => $val) {
               ...
               $headers_line .= Search::showHeaderItem($output_type,$searchopt["PluginResourcesDirectory"][$val]["name"],
            ...
            if ($output_type==Search::HTML_OUTPUT) {
               echo "<br>";
               Html::printPager($p['start'], $numrows, $target, $parameters, "PluginResourcesDirectory", 0,
                                $search_config);
            }
         } else {
