...
class PluginIdeaboxProfile extends CommonDBTM {
   ...
   static function getTypeName($nb=0) {
      return __('Rights management', 'ideabox');
   }
   ...
   static function displayTabContentForItem(CommonGLPI $item, $tabnum=1, $withtemplate=0) {
      ...
      if ($item->getType()=='Profile') {
         ...
         $prof->showForm($item->getField('id'), array('target' => 
                           $CFG_GLPI["root_doc"]."/plugins/ideabox/front/profile.form.php"));
      }
      return true;
   }
   ...
   static function changeProfile() {
      ...
      if ($prof->getFromDBByProfile($_SESSION['glpiactiveprofile']['id'])) {
         $_SESSION["glpi_plugin_ideabox_profile"]=$prof->fields;
      } else {
         unset($_SESSION["glpi_plugin_ideabox_profile"]);
      }
   }
	...
	function showForm ($ID, $options=array()) {
      ...
      $this->showFormHeader($options);
		...
		echo "<th colspan='4'>".sprintf(__('%1$s - %2$s'), __('Rights management', 'ideabox'),
         $prof->fields["name"])."</th>";
      echo "</tr>";
      ...
      echo "<td>" . __('Associable items to a ticket') . " - " .
         PluginIdeaboxIdeabox::getTypeName(2) . "</td><td>";
