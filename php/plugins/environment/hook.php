...
function plugin_environment_install() {
   ...
   if(!TableExists("glpi_plugin_environment_profiles")) {
      ...
      $DB->runFile(GLPI_ROOT ."/plugins/environment/sql/empty-1.4.0.sql");

   } else if(TableExists("glpi_plugin_environment_profiles") && FieldExists("glpi_plugin_environment_profiles","interface")) {
      
      $update = true;
      $DB->runFile(GLPI_ROOT ."/plugins/environment/sql/empty-1.3.0.sql");
      $DB->runFile(GLPI_ROOT ."/plugins/environment/sql/empty-1.4.0.sql");

   } else if(TableExists("glpi_plugin_environment_profiles") && FieldExists("glpi_plugin_environment_profiles","connections")) {
...
function plugin_environment_uninstall() {
	...
	$query = "DROP TABLE IF EXISTS `glpi_plugin_environment_profiles`;";
	$DB->query($query);
	
	unset($_SESSION["glpi_plugin_environment_installed"]);
